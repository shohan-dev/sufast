...
class RouteEntry:
    ...
    def to_metadata(self) -> dict:
        ...
        summary = (
            self.summary
            or func_info.get("description", "")
            or f"{self.method} {self.path}"
        )
...
class Router:
    ...
    ) -> Optional[Tuple[RouteEntry, Dict[str, Any]]]:
...
class APIRouter:
    ...
    def _add_route(self, method: str, path: str, **kwargs):
        def decorator(func):
            self._routes.append(
                {"method": method, "path": path, "handler": func, **kwargs}
            )
            return func
    ...
    def websocket(self, path: str, **kwargs):
        def decorator(func):
            self._ws_routes.append({"path": path, "handler": func, **kwargs})
            return func
...
class Sufast:
    ...
    def _setup_rust_ffi(self):
        ...
        try:
            ...
            # Server start
            self._rust_core.start_ultra_fast_server.argtypes = [
                ctypes.c_char_p,
                ctypes.c_uint16,
            ]
            ...
            self._rust_core.get_performance_stats.restype = ctypes.POINTER(
                ctypes.c_char
            )
    ...
    def _add_route(
        self, method: str, path: str, handler: Callable, **kwargs
    ) -> Callable:
        ...
        if is_static is None:
            is_static = (
                not has_params and method == "GET" and kwargs.get("cache_ttl", 0) == 0
            )
    ...
    def _generate_openapi_spec(self) -> dict:
        ...
        if (
            self._openapi_cache
            and (now - self._openapi_cache_time) < self._openapi_cache_ttl
        ):
    ...
    def _register_health_endpoint(self):
        ...
        @self.get("/health", tags=["System"], group="System", summary="Health Check")
    ...
    async def _handle_request(
        ...
        async def call_handler():
            ...
            return Response(
                ...
                content_type=formatted.get("headers", {}).get(
                    "Content-Type", "application/json"
                ),
            )
    ...
    async def _invoke_handler(
        self, route: RouteEntry, request: Request, params: dict
    ) -> Any:
        ...
        for param_name, param in sig.parameters.items():
            if param_name in params:
                ...
                param_name == "background_tasks" or param.annotation is BackgroundTasks
            ):
    ...
    async def _handle_websocket(self, ws: WebSocket, path: str):
        ...
        for ws_route in self._ws_routes:
            ...
            if params is not None:
                ...
                for pname in sig.parameters:
                    if (
                        pname in ("websocket", "ws")
                        or sig.parameters[pname].annotation is WebSocket
                    ):
                        kwargs[pname] = ws
    ...
    async def _serve_static_file(
        self, path: str, mount_path: str, directory: str
    ) -> dict:
        """Serve a static file."""
        import mimetypes
    ...
    def run(
        ...
        **kwargs,
        ...
        _safe_console_print(
            f"  \033[1;36m⚡ Sufast v{self.version}\033[0m - {self.title}"
        )
        if self._rust_available:
            _safe_console_print(
